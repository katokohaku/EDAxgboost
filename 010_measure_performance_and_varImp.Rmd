...
```{r explainer.DALEX}
predict.classif <- function(model, x) {
  pred.x <- predict(model, x)
  ifelse(pred.x > 0.5, 1, 0)
}
# predict.classif(model.xgb, train.xgb.DMatrix)
explainer.xgb <- DALEX::explain(model.xgb, 
                                data  = test.matrix, 
                                y     = test.label, 
                                predict_function = predict.classif,
                                label = "xgboost")
